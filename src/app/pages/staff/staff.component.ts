...
export class StaffComponent {
  ...
  get isDriver(): boolean { return this.form.designation?.trim().toLowerCase() === 'driver'; }
  ...
  onDesignationSelected(value: string, panel: TemplateRef<unknown>): void { this.markInteracted('designation'); if (value !== this.quickAddDesignationValue) { this.onDesignationChange(); return; } this.form.designation = ''; this.quickDesignationName = ''; this.offcanvas.open(panel, { position: 'end', backdrop: false, panelClass: 'custom-subscription-offcanvas' }); }
  ...
  fieldError(field: 'phone_number' | 'employment_date' | 'date_of_birth' | 'permit_number' | 'permit_expiry_date'): string | null {
    ...
    if (field === 'permit_number' && this.form.permit_expiry_date && !this.form.permit_number?.trim()) return 'Permit number is required when expiry is entered.';
    if (field === 'permit_expiry_date' && this.form.permit_number?.trim() && !this.form.permit_expiry_date) return 'Permit expiry is required when permit number is entered.';
    return null;
  }
  private validateForm(): string | null {
    ...
    if (this.form.permit_number?.trim() && !/^\d{8}$/.test(this.form.permit_number.trim())) return 'Driving permit number must contain exactly 8 digits.';
    if (this.form.permit_number?.trim() && !this.form.permit_expiry_date) return 'Enter the permit expiry date when a permit number is provided.';
    if (this.form.permit_expiry_date && !this.form.permit_number?.trim()) return 'Enter the permit number when a permit expiry date is provided.';
    return null;
  }
